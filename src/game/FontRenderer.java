...
public class FontRenderer {
	...
	public static void drawString(String msg, Gui gui, int x, int y, int color, int scale)
	{
		int textWidth = 0;
		...
		for (int i = 0; i < msg.length(); i++) {
			...
			if (charIndex < 0)
				charIndex = 29;
			GuiRenderer.render(gui, FONT1, x + textWidth + scale * i, y, charIndex, Colour.get(-1, -1, -1, color),
					0x00, scale);
			textWidth = textWidth + charWidth * scale;
		}
	}

	public static void drawCenteredString(String msg, Gui gui, int x, int y, int color, int scale)
	{
